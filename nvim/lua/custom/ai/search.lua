...
local function safe_set_lines(buf, arg2, arg3, arg4, arg5)
	local start_idx = 0
	local end_idx = -1
	local raw_lines = nil

	-- 1. 自动适配参数格式：
	-- 格式 A: safe_set_lines(buf, lines)
	if arg3 == nil then
		raw_lines = arg2
	-- 格式 B: safe_set_lines(buf, 0, -1, false, lines)  (原版 5 参数)
	elseif arg5 ~= nil then
		start_idx = arg2
		end_idx = arg3
		raw_lines = arg5
	-- 格式 C: safe_set_lines(buf, 0, -1, lines)         (封装版 4 参数)
	else
		start_idx = arg2
		end_idx = arg3
		raw_lines = arg4
	...
	-- 2. 自动把单字符串或含换行符的内容拆解成合法的单行 table
	local lines = {}
	if type(raw_lines) == "string" then
		for _, l in ipairs(vim.split(raw_lines, "\n", { plain = true })) do
			table.insert(lines, l)
		end
	elseif type(raw_lines) == "table" then
		for _, item in ipairs(raw_lines) do
			if type(item) == "string" and item:find("\n") then
				for _, sub_line in ipairs(vim.split(item, "\n", { plain = true })) do
...
2. Convert the user's natural language question into exactly 3 dense, distinct keyword queries for search engines.
   ...
   - Don't assume anything that could be outdated (e.g. Time, Software versions, etc) unless explicitly asked by user.
				...
				vim.schedule(function()
					local q_preview = {
						...
						string.format(
							"> 🧭 **Strategy Mode:** `%s` (%s)",
							search_mode,
							search_mode == "precision" and "Single-Source Precision" or "Multi-Source Fusion"
						),
						"",
					...
					safe_set_lines(target_buf, 0, -1, false, q_preview)
				end)
					...
					fetch_searxng_single(q_text, function(items)
						...
						if completed == total_searches then
							...
							if #merged_candidates == 0 then
								vim.schedule(function()
									local debug_info =
										{ "❌ No search results found across all queries.", "", "Searched Queries:" }
									for idx, q in ipairs(generated_queries) do
										table.insert(
											debug_info,
											string.format(
												"- Query %d: `%s` (Returned: %d results)",
												idx,
												q,
												#(all_results[idx] or {})
											)
									...
									safe_set_lines(target_buf, 0, -1, false, debug_info)
								end)
								return
							end

							local sources_header =
								{ string.format("### 🎯 Optimized Search Queries (Mode: %s):", search_mode:upper()) }
							for i, q in ipairs(generated_queries) do
							...
							for _, item in ipairs(merged_candidates) do
								table.insert(
									sources_header,
									string.format("- [%d] [%s](%s)", item.id, item.title, item.url)
								)
							end
							table.insert(sources_header, "")
							table.insert(sources_header, "---")
							...
							vim.schedule(function()
								safe_set_lines(target_buf, 0, -1, false, {
									"# 🔍 Web Search: `" .. prompt .. "`",
									"",
							...
							local sl_chunks = {}
							vim.fn.jobstart(
								{ "curl", "-s", "-X", "POST", OLLAMA_HOST .. "/api/chat", "-d", shortlist_payload },
								{
									stdout_buffered = true,
									on_stdout = function(_, data)
										if data then
											vim.list_extend(sl_chunks, data)
										end
									end,
									on_exit = function()
										...
										local seen_id = {}
										local unique_shortlisted = {}
										for _, id in ipairs(shortlisted_ids) do
											if not seen_id[id] and merged_candidates[id] then
												seen_id[id] = true
												table.insert(unique_shortlisted, id)
											end
										end
										shortlisted_ids = unique_shortlisted

										if #shortlisted_ids == 0 then
											for i = 1, math.min(3, #merged_candidates) do
												table.insert(shortlisted_ids, i)
											end
										end
										while #shortlisted_ids > 3 do
											table.remove(shortlisted_ids)
										end
										...
										vim.schedule(function()
											safe_set_lines(target_buf, 0, -1, false, {
												...
												"",
												string.format(
													"[4/5] Concurrently downloading shortlisted sources: `[%s]`...",
													table.concat(shortlisted_ids, ", ")
												),
											})
										end)
											...
											fetch_page_content(s_item.url, function(p_content)
												fetched_pages[sid] = {
													...
													preview = string.format(
														'<candidate id="%d" title="%s" url="%s">\n%s\n</candidate>',
														sid,
														s_item.title,
														s_item.url,
														p_content:sub(1, 2800)
													),
												}
												...
												if pages_done == total_to_fetch then
													...
													if search_mode == "precision" then
														...
														for _, pid in ipairs(shortlisted_ids) do
															if fetched_pages[pid] then
																table.insert(
																	tournament_previews,
																	fetched_pages[pid].preview
																)
															end
														end
...
Output ONLY the single integer ID of the best candidate. No other text.]],
																},
																{
																	...
																	content = string.format(
																		"User Query: %s\n\nCandidate Previews:\n%s\n\nBest Source Number:",
																		user_query,
																		table.concat(tournament_previews, "\n\n")
																	),
																},
															},
														...
														vim.fn.jobstart({
															...
															on_stdout = function(_, data)
																if data then
																	vim.list_extend(fin_chunks, data)
																end
															end,
															on_exit = function()
																local fin_raw = table.concat(fin_chunks, "")
																local ok_fin, res_fin =
																	pcall(vim.fn.json_decode, fin_raw)
																local raw_content = (
																	ok_fin
																	and res_fin.message
																	and res_fin.message.content
																) or ""
																local fin_text = strip_think(raw_content)

																local champ_id = tonumber(fin_text:match("%d+"))
																	or shortlisted_ids[1]
																if not fetched_pages[champ_id] then
																	champ_id = shortlisted_ids[1]
																end
																local champion_page = fetched_pages[champ_id]
																	or fetched_pages[shortlisted_ids[1]]
																if not champion_page then
																	champion_page = {
																...
																vim.schedule(function()
																	...
																	for _, l in ipairs(sources_header) do
																		table.insert(progress_view, l)
																	end
																	table.insert(
																		progress_view,
																		string.format(
																			"> 🎯 **[Precision] Selected Reference [%d]:** [%s](%s)",
																			champ_id,
																			champion_page.item.title,
																			champion_page.item.url
																		)
																	)
																	...
																	table.insert(
																		progress_view,
																		"⏳ Synthesizing accurate answer from champion source..."
																	)
																	safe_set_lines(
																		target_buf,
																		0,
   ...
   - ABSOLUTELY NEVER write your own markdown links or invent URLs (e.g. NEVER write [Text](http://...)).]],
																		},
																		{
																			...
																			content = string.format(
																				...
																				user_query,
																				champion_page.item.url,
																				champion_page.full_text:sub(1, 8000)
																			),
																		},
																	},
																...
																vim.fn.jobstart({
																	...
																	on_stdout = function(_, data)
																		if data then
																			vim.list_extend(ans_chunks, data)
																		end
																	end,
																	on_exit = function()
																		local ans_raw = table.concat(ans_chunks, "")
																		local ok_ans, res_ans =
																			pcall(vim.fn.json_decode, ans_raw)
																		vim.schedule(function()
																			if
																				ok_ans
																				and res_ans.message
																				and res_ans.message.content
																			then
																				local answer =
																					strip_think(res_ans.message.content)
																				...
																				for _, l in ipairs(sources_header) do
																					table.insert(full_view, l)
																				end
																				table.insert(
																					full_view,
																					string.format(
																						"> 📋 **AI 3-Finalist Shortlist:** %s",
																						table.concat(
																							shortlisted_text,
																							" | "
																						)
																				...
																				table.insert(
																					full_view,
																					string.format(
																						"> 🏆 **[Precision] Tournament Champion:** `[%d]` [%s](%s)",
																						champ_id,
																						champion_page.item.title,
																						champion_page.item.url
																					)
																				...
																				print(
																					"✓ Precision Search & Synthesis completed!"
																				)
																			else
																				safe_set_lines(
																					target_buf,
														...
														for _, pid in ipairs(shortlisted_ids) do
															...
															if doc_data then
																...
																table.insert(
																	combined_docs,
																	string.format(
																		'<document id="%d" title="%s" url="%s">\n%s\n</document>',
																		pid,
																		doc_data.item.title,
																		doc_data.item.url,
																		doc_data.full_text:sub(1, 3500)
																	)
																)
																table.insert(
																	reading_list_text,
															...
															on_exit = function()
																...
																vim.schedule(function()
																	...
																	then
																		...
																		table.insert(
																			full_view,
																			"> 🌐 **[Synthesis] Multi-Document Fusion:**"
																		)
																		for _, doc_line in ipairs(reading_list_text) do
																			table.insert(full_view, "> " .. doc_line)
																		end
																		table.insert(full_view, "")
																		...
																		local err_lines = { "❌ Synthesis error:" }
																		for _, l in ipairs(vim.split(ans_raw, "\n")) do
